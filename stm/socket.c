...
#define assert(x) { if (!(x)) { print(env, "FATAL ERROR: Failed assert at line %d\n\r", __LINE__) } }
...
static machine_int_t socket_read(mp_obj_t o_in, void *buf, machine_uint_t size, int *errcode) {
    ...
    machine_int_t r = pico_read(o->fd, buf, size);
    if (r == -1) {
        *errcode = pico_err;
    }
    return r;
...
static machine_int_t socket_write(mp_obj_t o_in, const void *buf, machine_uint_t size, int *errcode) {
    mp_obj_socket_t *o = o_in;
    machine_int_t r = pico_write(o->fd, buf, size);
    if (r == -1) {
        *errcode = pico_err;
    }
    return r;
...
static mp_obj_t socket_connect(mp_obj_t self_in, mp_obj_t addr_in) {
    ...
    int r = pico_connect(self->fd, (const struct sockaddr *)bufinfo.buf, bufinfo.len);
    RAISE_ERRNO(r, pico_err);
    return mp_const_none;
}
...
static mp_obj_t socket_bind(mp_obj_t self_in, mp_obj_t addr_in) {
    ...
    buffer_info_t bufinfo;
    get_buffer(addr_in, &bufinfo);
    int r = pico_bind(self->fd, (const struct sockaddr *)bufinfo.buf, bufinfo.len);
    RAISE_ERRNO(r, pico_err);
    return mp_const_none;
}
...
static mp_obj_t socket_listen(mp_obj_t self_in, mp_obj_t backlog_in) {
    mp_obj_socket_t *self = self_in;
    int r = pico_listen(self->fd, MP_OBJ_SMALL_INT_VALUE(backlog_in));
    RAISE_ERRNO(r, pico_err);
    return mp_const_none;
}
...
static mp_obj_t socket_accept(mp_obj_t self_in) {
    ...
    int fd = pico_accept(self->fd, &addr, &addr_len);
    RAISE_ERRNO(fd, pico_err);
...
static mp_obj_t socket_recv(uint n_args, const mp_obj_t *args) {
    ...
    int out_sz = pico_recv(self->fd, buf, sz, flags);
    RAISE_ERRNO(out_sz, pico_err);
...
static mp_obj_t socket_send(uint n_args, const mp_obj_t *args) {
    ...
    int out_sz = pico_send(self->fd, buf, sz, flags);
    RAISE_ERRNO(out_sz, pico_err);
...
static mp_obj_t socket_make_new(mp_obj_t type_in, uint n_args, uint n_kw, const mp_obj_t *args) {
    ...
    int fd = pico_newsocket(family, type, proto);
    RAISE_ERRNO(fd, pico_err);
...
static mp_obj_t mod_socket_gethostbyname(mp_obj_t arg) {
    ...
    if (h == NULL) {
        nlr_jump(mp_obj_new_exception_msg_varg(&mp_type_OSError, "[Errno %d]", pico_err));
    }
    assert(h->h_length == 4);
